...
final class RegistrationPasswordViewController: CommonViewController {
	...
	private lazy var subtitleLabel: UILabel = {
		...
		label.text = "Потребуется при авторизации"
		label.font = .standart(style: .regular, of: 14)
		label.textColor = .textSecondary
		return label
	}()
	private let textField: CommonTextField = {
		let textField = CommonTextField(isPassword: true)
...
private extension RegistrationPasswordViewController {
	...
	func buttonAction() {
		guard let text = textField.text else {
			textField.validationEnded(with: .error(message: "Введите пароль"))
			return
		}
		button.isLoading = true
		presenter.askIfCanContinue(with: .password, text: text) { [weak self] result in
			guard let self = self else { return }
			self.button.isLoading = false
			if result {
				self.textField.validationEnded(with: .correct)
			} else {
				self.textField.validationEnded(with: .error(message: "Что-то не так с паролем"))
			}
		}
	}
}
