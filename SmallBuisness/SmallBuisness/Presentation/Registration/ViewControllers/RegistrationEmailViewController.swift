...
final class RegistrationEmailViewController: CommonViewController {
	private let presenter: RegistrationPresenter
	private lazy var titleLabel: UILabel = {
		let label = UILabel()
		label.text = "Добавьте почту"
	...
	private lazy var subtitleLabel: UILabel = {
		...
		label.text = "При помощи почты Вы сможете восстановить аккаунт при необходимости"
...
private extension RegistrationEmailViewController {
	...
	func buttonAction() {
		guard let text = textField.text else {
			textField.validationEnded(with: .error(message: "Введите почту"))
			return
		}
		button.isLoading = true
		presenter.askIfCanContinue(with: .email, text: text) { [weak self] result in
			guard let self = self else { return }
			self.button.isLoading = false
			if result {
				self.textField.validationEnded(with: .correct)
			} else {
				self.textField.validationEnded(with: .error(message: "Почта занята"))
			}
		}
	}
}
