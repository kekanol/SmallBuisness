...
final class RegistrationNameViewController: CommonViewController {
	...
	private lazy var subtitleLabel: UILabel = {
		...
		label.text = "Ваше уникальное имя в социальной сети. Вы можете поменять его позже"
...
private extension RegistrationNameViewController {
	...
	func buttonAction() {
		guard let text = textField.text else {
			textField.validationEnded(with: .error(message: "Введите имя"))
			return
		}
		button.isLoading = true
		presenter.askIfCanContinue(with: .name, text: text) { [weak self] result in
			guard let self = self else { return }
			self.button.isLoading = false
			if result {
				self.textField.validationEnded(with: .correct)
			} else {
				self.textField.validationEnded(with: .error(message: "Данное имя уже занято"))
			}
		}
	}
}
